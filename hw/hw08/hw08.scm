(define (accumulate-tail combiner start n term)
  ...
  (define (helper res n)
          (if (= n 0) res
              (helper (combiner res (term n)) (- n 1))
          )
  )
  (helper start n)
)
...
(define (make-exp base exponent) 'YOUR-CODE-HERE
    (cond 
      ...
      ((and (number? base) (number? exponent)) (* base (make-exp base (- exponent 1))))
...
(define (exp? exp) 'YOUR-CODE-HERE
      (cond
        ((number? exp) #f)
        ((variable? exp) #f)
        ((and (eqv? (car exp) '^) (not (null? (first-operand exp))) (not (null? (second-operand exp)))) #t)
        (else #f)
      )
)
