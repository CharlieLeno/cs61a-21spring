...
def path_yielder(t, value):
    ...
    def dfs(t, path):
        ...
        for branch in t.branches:
            yield from dfs(branch, path + [branch.label])
    yield from dfs(t, [t.label])

    """
    if t.label == value:
        yield [t.label]
    for branch in t.branches:
        for path in list(path_yielder(branch, value)):
            yield [t.label] + path
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(t, minLimit, maxLimit):
        if t.label < minLimit or t.label > maxLimit:
            return False
        # 0 children
        if t.is_leaf():
            return True
        #one children
        elif len(t.branches) == 1:    #one children
            # left children or right children
            if helper(t.branches[0], minLimit, t.label) or helper(t.branches[0], t.label + 1, maxLimit):
                return True 
            return False
        #two children
        elif len(t.branches) == 2:
            if t.branches[0].label <= t.label < t.branches[1].label:
                if helper(t.branches[0], minLimit, t.label) and helper(t.branches[1], t.label + 1, maxLimit):
                    return True
                return False
            return False           
