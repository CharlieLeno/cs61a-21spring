...
def choose(paragraphs, select, k):
    ...
    def helper(i, index):
        if index == len(paragraphs):
            return ""
        if not select(paragraphs[index]):
            return helper(i, index + 1)
        else:
            if i == k:
                return paragraphs[index]
            else:
                return helper(i + 1, index + 1)
    return helper(0, 0)
"""
    index = 0
    while  k >= 0 and index < len(paragraphs):
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def isWordsContained(para):
        para_form = split(lower(remove_punctuation(para)))
        for word_para in para_form:
            for word_topic in topic:
                if word_para == word_topic:
                    return True
        return False
    return isWordsContained
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        diff_num = diff_function(typed_word, word, limit)
        if diff_num < diff_num_min:
            ret_word = word
            diff_num_min = diff_num  
        elif diff_num == limit and ret_word == typed_word:
            "the first word meet limit"
            ret_word = word
    return ret_word
...
def sphinx_switches(start, goal, limit):
    ...
    # BEGIN PROBLEM 6
    """
    n = min(len(start), len(goal))
    def helper(index, diff_num):
        if diff_num > limit or index == n:
            return diff_num
        if start[index] != goal[index]:
            return helper(index + 1, diff_num + 1)
        return helper(index + 1, diff_num)
    return helper(0, abs(len(start) - len(goal)))
    """
...
def pawssible_patches(start, goal, limit):
    ...
    if start == goal or limit < 0:  # Fill in the condition
        ...
        if start[0] != goal[0]:
            add = goal[0] + start[0:]  # Fill in these lines
            remove = start[1:]
            substitute = goal[0] + start[1:]
            num_add = pawssible_patches(add, goal, limit - 1)
            num_remove = pawssible_patches(remove, goal, limit - 1)
            num_substitute = pawssible_patches(substitute, goal, limit - 1)
            return 1 + min(num_add, num_remove, num_substitute)
        else:
...
def report_progress(typed, prompt, user_id, send):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def count_ratio(typed, prompt):
        for i in range(len(typed)):
            if typed[i] != prompt[i]:
                return i / len(prompt)
        return len(typed) / len(prompt)
    ratio = count_ratio(typed, prompt)
...
def time_per_word(times_per_player, words):
    ...
    num_words = len(words)
    times = [[] for _ in range(num_player)]
    for i in range(num_player):
        for j in range(num_words):
            times[i] += [times_per_player[i][j + 1] - times_per_player[i][j]]
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    words_fast = [[] for _ in player_indices]
    for i in word_indices:
        time_fast = time(game, 0, i)
        player_fast = 0
        for j in player_indices:
            if time(game, j, i) < time_fast:
                time_fast = time(game, j, i)
                player_fast = j
        words_fast[player_fast] += [word_at(game, i)]
    return words_fast
    # END PROBLEM 10
