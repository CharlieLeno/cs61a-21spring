...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if self.is_container():
            contained_ant = self.contained_ant
            if contained_ant:
                contained_ant.buff()
        if not self.buff_flag:
            self.buff_flag = True
            self.damage  *= 2
        # END Problem EC
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not curr.is_hive():
            if self.min_range <= cnt <= self.max_range and curr.bees:
                return bee_selector(curr.bees)
            curr = curr.entrance
            cnt += 1
        return None
        # END Problem 3 and 4
...
# END Problem EC
    ...
    def __init__(self, health=1):
        # BEGIN Problem EC
        super().__init__(health)
        self.impostor = QueenAnt.exist_status
        QueenAnt.exist_status = True
        # END Problem EC
...
class Bee(Insect):
    ...
    def __init__(self, health = 1):
        ...
        self.direction = 0   #0 means left, 1 means right
    ...
    def action(self, gamestate):
        ...
        print("DEBUG:", "normal",self, self.slow_count, self.scare_count, self.direction)
        if self.blocked():
    ...
    def slow(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        self.apply_status('slow', self.action, length)
        # END Problem Optional 2
    ...
    def scare(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if self.scared_before:
            return
        self.scared_before = True
        self.apply_status('scare', self.action, length)
        # END Problem Optional 2
    ...
    def apply_status(self, status, previous_action, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        def action_slow(gamestate):
            if gamestate.time % 2 == 0:
                print("DEBUG:", "slow", self.slow_count)
                previous_action(gamestate)
            self.slow_count += 1
            if self.slow_count == length:
                self.slow_count = 0
                self.action = previous_action

        def action_scare(gamestate):
            print("DEBUG:", "scare", self.direction)
            self.direction = 1
            previous_action(gamestate)
            self.scare_count += 1
            if self.scare_count == length:
                self.scare_count = 0
                self.direction = 0
                self.action = previous_action
            print("DEBUG:", "scare",self, self.slow_count, self.scare_count, self.direction)
        
        print("DEBUG:", "3",status)
        if status == 'slow':
            self.action = action_slow
        elif status == 'scare':
            self.action = action_scare
        else:
            return
        # END Problem Optional 2
