...
def eval_all(expressions, env):
    ...
    while curr is not nil:
        res = scheme_eval(curr.first, env)
        curr = curr.rest
    return res
    # END PROBLEM 7
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        currForm = formals
        currVal = vals
        while currForm is not nil and currVal is not nil:
            child.define(currForm.first, currVal.first)
            currForm = currForm.rest
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        curr = args
        while curr is not nil:
            arguments_list.append(curr.first)
            curr = curr.rest
        if self.use_env:
