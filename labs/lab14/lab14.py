...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    branch1 = t.branches[0]
    branch2 = t.branches[1]
    if branch1.label > branch2.label:
        t.branches.pop(0)
    else:
        t.branches.pop(1)
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(s, downLimit, upLimit):
        if len(s) == 1:
            if downLimit <= s[0] <= upLimit:
                return 1
            else:
                return 0
        return helper(s[1:], d - abs(s[0]), d + abs(s[0]))
    helper(s, d, d)
