def insert_into_all(item, nested_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_list = [[] for _ in range(len(nested_list))]
    for i in range(len(nested_list)):
        new_list[i] = [item] + nested_list[i]
    return new_list
...
def subseqs(s):
    ...
    if not s:
        ...
        res = subseqs(s[1:])
        return insert_into_all(s[0], res) + res
...
def insert(link, value, index):
    ...
    node = Link(value)
    curr.rest, node.rest = node, curr.rest
...
def long_paths(t, n):
    ...
    "*** YOUR CODE HERE ***"
    res = []
    def helper(t, length, path):
        if t.is_leaf() and length >= n:
            res.append(path)
        for branch in t.branches:
            helper(branch, length + 1, path + [branch.label])
    helper(t, 0, [t.label])
    return res
