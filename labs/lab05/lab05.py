...
def closer_city(lat, lon, city_a, city_b):
    ...
    a_distance = distance(city_a, temp)
    b_distance = distance(city_b, temp)
    return get_name(city_a) if a_distance < b_distance else get_name(city_b)
...
def add_trees(t1, t2):
    ...
    b = branches(t2)
    if len(a) < len(b):
        a += [tree(0)] * (len(b) - len(a))
    elif len(a) > len(b):
        b += [tree(0)] * (len(a) - len(b))
    branch = [add_trees(x, y) for x, y in zip(a, b)]
